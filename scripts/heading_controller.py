...
class HeadingController(BaseHeadingController):
    ...
    def compute_control_with_goal(
        ...
        theta_error = wrap_angle(goal.theta - state.theta)

        tolerance = float(self.get_parameter("heading_tolerance").value)
        if abs(theta_error) < tolerance:
            command.omega = 0.0
        else:
            # apply proportional control on the wrapped heading error
            command.omega = float(self.kp * theta_error)
